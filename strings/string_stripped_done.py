...
def donuts(count):
    ...
    return result.format(count) if count < 10 else result.format('many')
...
def mix_up(a, b):
    return '{0}{1} {2}{3}'.format(b[0:2], a[2:], a[0:2], b[2:])
...
def not_bad(s):
    ...
    r_index = s.find('bad')
    if l_index < r_index:
        s = s.replace(s[l_index: r_index + 3], 'good')
    return s
...
def front_back(a, b):
    first_half = divide_string(a)
    second_half = divide_string(b)
    return first_half[0]+second_half[0]+first_half[1]+second_half[1]
